...
//---flag variable used to represent the graph---//
...
found:
  p->state = EMBRYO;
  p->pid = nextpid++;
  p->priority = 2;	//project2,priority modification default=2
  p->lr=0;		 	//project2,lastrunned is 0.
  release(&ptable.lock);
...
//---this function helps us to choose between round robin and priority based scheduler---//
...
sched_type(int n)
{
 if(n==0||n==1)  
 {
  type=n;
  if(n==0)
  cprintf("scheduler selected:RoundRobin scheduler\n");
  else
  cprintf("scheduler selected:Priority based scheduler\n");
  return 0;
  }
 else return -1;
}
...
//-----*project2*-----//
  if (flag)
  {
    cprintf("%d (%d)| ", proc->pid,proc->priority);
  }
...
{
    //    cprintf("in clone proc.c, stack start addr %p\n", stack);
    /*
    ...
    *np->tf = *proc->tf; // Change new child's tf
    ...
    // np->tf->eip np->tf->eip
    *ustack = (int) arg; 
    *(ustack - 4) = 0xffffffff; // bs return addr
   //  *(ustack - 2) = 0xffffffff; // bs ebp
    ...
    np->state = RUNNABLE;
    safestrcpy(np->name, proc->name, sizeof(proc->name));
    //cprintf("clone about to return pid %d\n", pid);
    return pid;
...
join(void ** stack)
{
    //cprintf("in join proc.c with stackptr @ %p\n", *stack); 
    struct proc *p;
    int havekids, pid;
    acquire(&ptable.lock);
    for(;;)
